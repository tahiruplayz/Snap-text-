...
import {
  ...
  History, ChevronLeft, ChevronRight, LogOut, LogIn, User, Zap,
} from 'lucide-react';
import { useAuth } from '../context/AuthContext';

const NAV = [
  { to: '/',          icon: ScanText,   label: 'OCR Extract' },
  { to: '/clean',     icon: Sparkles,   label: 'Clean Text' },
  { to: '/notes',     icon: BookOpen,   label: 'Notes Generator' },
  { to: '/pdf',       icon: FileText,   label: 'Image to PDF' },
...
export default function AppShell() {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-surface">
      {/* ── Sidebar ── */}
      <aside
        className={`flex flex-col flex-shrink-0 bg-surface-2 border-r border-surface-3
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Top bar */}
        <header className="h-16 flex-shrink-0 bg-surface-2/60 backdrop-blur-sm border-b border-surface-3 flex items-center px-6 gap-4">
